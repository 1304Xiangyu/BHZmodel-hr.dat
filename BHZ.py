...
while l<=Nk1-1:
    while m<=Nk2-1:
        Hk[:,:,l,m] = ([u+math.cos(kx[l])+math.cos(ky[m]), -math.sin(ky[m])*1j+ math.sin(kx[l]), 0, 0],
              [math.sin(ky[m])*1j+math.sin(kx[l]), -(u+math.cos(kx[l])+math.cos(ky[m])), 0, 0],
              [0, 0, u+math.cos(kx[l])+math.cos(ky[m]), -math.sin(ky[m])*1j-math.sin(kx[l])],
              [0, 0, math.sin(ky[m])*1j-math.sin(kx[l]), -(u+math.cos(kx[l])+math.cos(ky[m]))])
