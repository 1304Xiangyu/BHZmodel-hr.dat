...
while l<=Nk1-1:
    while m<=Nk2-1:
        Hk[:,:,l,m] = ([u+math.cos(kx[l])+math.cos(ky[m]), -math.sin(ky[m])*1j+ math.sin(kx[l]), 0, -0.3*1j],
              [math.sin(ky[m])*1j+math.sin(kx[l]), -(u+math.cos(kx[l])+math.cos(ky[m])), 0.3*1j, 0],
              [0, -0.3*1j, u+math.cos(kx[l])+math.cos(ky[m]), -math.sin(ky[m])*1j-math.sin(kx[l])],
              [0.3*1j, 0, math.sin(ky[m])*1j-math.sin(kx[l]), -(u+math.cos(kx[l])+math.cos(ky[m]))])
...
f = open('BHZ-antisymmetric_hr.dat','w')
f.write("Hr_dat for BHZ model with C = 0.3sigma_y"+'\n')      # Comment line
...
plt.savefig("kx_band-antisymmetric.png")
plt.close()
#plt.show()
...
plt.savefig("band_surf-antisymmetric.png")
